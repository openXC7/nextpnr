# 02 — Fork feature inventory: `nextpnr-xilinx` (openXC7) feature reference (blank-filler)

> Draft. Read-only analysis of `/devel/HDL/kintex-reveng/nextpnr-xilinx` @ `68aeeb39`
> (branch `main`, tag `0.9.3`). This document inventories the fork's complete
> feature set so document 04 can diff it against upstream `himbaechel/uarch/xilinx`.
