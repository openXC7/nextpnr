...
> Draft. Read-only comparison of the placement / routing / legalisation ALGORITHMS.
>
...
> - **FORK (feature reference (blank-filler))** = `/devel/HDL/kintex-reveng/nextpnr-xilinx` @ `68aeeb39`
...
>
> **The decision being supported**: keep the upstream (himbaechel) placer/router/
> legaliser, port the fork's *legality data + missing passes* into it, and understand
> every algorithmic divergence so porting does not reintroduce bugs or silently drop
> the fork's placement-quality work.
...
6. Main loop `while (stalled < 5 && solved_hpwl <= legal_hpwl*0.8)` (`:249`): for each
   **bel-bucket run** (`heap_runs`, built at `:218-242`; `placeAllAtOnce` collapses to
   ...
   - `legalise_placement_strict()` (`StrictLegaliser` `:1067-1477`) — a **greedy
     largest-macro-first random-walk** legaliser (the *same* family as the fork's,
     not bipartite matching), augmented with control-set pre-search
     (`alloc_control_sets`/`find_control_set_candidates`), `chainRipup` whole-cluster
     ripup, `cell_placement_timeout`, and `get_cell_legalisation_weight` priority.
...
- **Bidirectional** meet-in-the-middle A* (fwd+bwd queues, `midpoint_wire`), historical
  + present congestion, per-net bounding box (expanded every 3 failures),
  resource/GroupId value tracking (`getResourceKeyForPip/getResourceValueForPip` +
  `PerResourceData`).
- **Wire reservation** (0.11 superset): `reserved_net` +
  `reserve_driver_wires_for_arc`/`reserve_sink_wires_for_arc` (`:597-684`) +
  `find_all_reserved_wires()` (`:686`), **plus resource/GroupId reservation**
  (`setup_resources` `:256-284`). On contention upstream `log_error`s hard.
- Timing via shared `TimingAnalyser` (clock-skew enabled, `tmg.run()` per iteration).
- Multithreaded 4-quadrant partition; always finishes with a `router1()` legality pass.
...
|---|---|---|
| Greedy **largest-macro-first** (priority queue keyed by `chain_size`) | `:896-899` | `StrictLegaliser` — *same* greedy walk, but adds control-set pre-search, `chainRipup`, `cell_placement_timeout`, `get_cell_legalisation_weight` (`placer_heap.cc:1067-1477`) |
...
| **Eviction as last resort** (`scarce_type` = type population ≤4096; only evict when no free valid home was seen; `bestFreeBel`/`bestBel` split) | `:923-932, 946-955, 1087-1107` | no equivalent upstream — worth porting as a termination/quality guarantee (corrigendum) |
...
| **Relocation of pre-bound leaf chain members** (legalisation unbinds a bound leaf `NPNR_ASSERT(vc->constr_children.empty())` and relocates it with the cluster) | `:1187-1201` | upstream `try_place_cluster` uses `getClusterPlacement`; no leaf-relocation case |
| **`place_constraints` chain-child binding** (`bind_children` binds `constr_children` of a BEL-pinned parent, imposing the parent's slice half on absolute z) | `:419-459` | upstream `place_constraints` (`placer_heap.cc:463-505`) stops at `bindBel(...,STRENGTH_USER)`; no child walk |
| Ends with `placer1_refine` | `:320` | `parallel_refine` *or* `placer1_refine` (`:401-412`) |

Diff verdict: upstream's `StrictLegaliser` is the **same greedy random-walk family**
as the fork (not bipartite matching), but re-implemented over ClusterId and augmented
with control-set pre-placement, `chainRipup`, `cell_placement_timeout`, and
`get_cell_legalisation_weight`. The fork's **fail-fast (`attempts` budget), eviction
as last resort (`bestFreeBel` + scarce-type), and deterministic full-scan fallback**
are robustness fixes for that greedy walk that upstream 0.11 **does not** have
(upstream evicts as soon as `bestBel` is found and has no full-scan fallback). These
are the fork's genuine, still-relevant contributions to *this* legaliser family — port
their *intent* into `StrictLegaliser`: a never-reset per-cell attempts cap that
includes a full-grid term, only-evict-when-no-free-home, and a deterministic free-bel
sweep for singleton site types.
...
Other fork-only placer1 details:
- **BEL-pinned chain-child pre-binding** (`bind_children` + `clear_constrs` block
  `:202-269`, incl. the xc7 z-half fix `:220-222`) — hybrid-flow support upstream lacks.
- **Chain swap is singleton-only** (`discover_chain` + `swap_cell_bels`, rejects
  swapping against another chain, `:729-811`). Upstream `try_swap_chain`
  (`placer1.cc:592-732`) can **displace an entire colliding cluster** recursively — a
  real QoR difference in upstream's favour.
- `NEXTPNR_EXCLUDE_STAMPED_BBOX` exclusion bbox (`:820-854`) — hybrid-flow only.
- Final `get_constraints_distance == 0` assertion (`:512-531`).
...
data (`getBudgetOverride`, `assign_budget`) as the fork's route() already does. Note the
timing-API gap: the fork drives SA from `PortRef.budget` + `get_criticalities` +
`predictDelay`, upstream from `TimingAnalyser` + `get_criticality` + `predictArcDelay`.
...
- **Search direction**: fork router2 is **forward-only A\*** with a separate backwards
  BFS merge pre-pass (`:765-850`); upstream is **bidirectional** meet-in-the-middle A*
  (`:724-1118`). This is the single biggest structural difference.
- **Wire reservation — correction to the premise**: BOTH have `reserved_net` +
  `find_all_reserved_wires()`; upstream 0.11.1 is a **superset** (driver-wire +
  sink-wire + resource/GroupId reservation, `router2.cc:256-284, 597-684`). What the
  fork *adds* is a more robust conflict-resolution policy, not reservation itself:
  `reserved_locked` (`:100`, for LOCKED bindings `:236`), a `RESERVED_CONTESTED` (−2)
  absorbing sentinel (`:79-94`) that lets the congestion loop arbitrate a contested
  corridor instead of `log_error`-ing (`:490-501`), and a 100-iteration fixpoint cap
  (`:526-546`). Upstream instead `log_error`s hard on contention (`router2.cc:616-619,
  660-663`) and has an uncapped fixpoint.
- Fork **const-net routing**: dedicated `route_xilinx_const` (`:571-725`, nearest
  pseudo-source backward BFS) and GND/VCC are deferred *out* of router2 to a later
  router1 `route_const_arc` pass (`:1454-1468`). Upstream routes const nets inline in
  router2 (`const_mode`).
- Fork **bounded termination**: stall/iter cap (`best_overuse`, `stall_iters`,
  `max_stall=50`, `max_iter=600`) + `NEXTPNR_SKIP_FAILED_ARCS` partial-route accept
  (`:1441-1553`); upstream loops `while(!failed_nets.empty())` unbounded.
- Fork router2 **final router1 recheck is opt-in** (`NPNR_ROUTER1_RECHECK`, `:1580`);
  upstream always runs it.
- `Router2Cfg` fork = upstream minus `heatmap`/`get_base_cost`.
...
2. **`legalise_placement_strict` greedy walk + fail-fast/fallback/eviction**
   (`common/placer_heap.cc:879-1180`) — upstream replaces with `StrictLegaliser`
   assignment + bel buckets. The fork's `ripup_radius` doubling, per-cell timeout,
   deterministic full-scan, and eviction-as-last-resort all exist to stop the *greedy*
   walk from livelocking. Only the *intent* (guaranteed termination, no gratuitous
   eviction, no singleton-site miss) needs re-verification against `StrictLegaliser`.
...
|---|---|---|---|---|
| placer_heap | HeAP + Eigen CG + CutSpreader + StrictLegaliser (greedy + control-set pre-search + `chainRipup` + `cell_placement_timeout`) + `parallel_refine`/`placer1_refine` | HeAP + greedy largest-macro-first legalisation + `chain_root/update_chain` + fail-fast/fallback/eviction | never-reset attempts cap, `bestFreeBel` eviction rule, deterministic full-scan fallback, leaf-member relocation | greedy legaliser body, `constr_*` walk, missing control-set API |
...
| router2 | bidirectional A* + driver/sink/resource reservation + `heatmap`/`get_base_cost` + always router1 recheck | forward-only A* + backward BFS merge; `route_xilinx_const`; `reserved_locked`/`RESERVED_CONTESTED`; bounded termination; opt-in recheck | `RESERVED_CONTESTED` policy + fixpoint cap + LOCKED keepout (if fixed-routes kept); bounded-termination/partial-route knobs | const-net deferral to router1 (upstream inline is fine) |
